...
본 프로젝트는 별도의 백엔드 서버 없이 동작하는 **Serverless 스타일 프론트엔드 구조**로 설계되었습니다.

- 실제 DB 서버 대신 `db.json`을 기반으로 동작하는 `json-server`를 Mock Backend로 사용
- RESTful API를 제공하는 `json-server`를 통해 프론트엔드에서 Axios로 CRUD 요청 처리
- `npm run start:all` 명령어로 프론트엔드(Vite)와 Mock API 서버를 동시에 실행

> 즉, **Vue 3 중심의 Frontend-Driven Architecture**
